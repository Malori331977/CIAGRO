    ...
    public class Consultas
    {
        ...
        public static bool listarDatosAuditoria(Listado listar, ref DataTable dt, ref string error)
        {
            ...
            try
            {
                string strSQL = " SELECT T1.UsuarioCreacionAdmin,T1.FechaCreacionAdmin,T1.UsuarioUltModificacionAdmin," +
                    ...
                    sqlCon.COMPAÑIA + ".NV_USUARIOS WHERE T1.UsuarioCreacionAdmin=CodigoUsuario) NombreUsuarioCreacion," +
                    "(SELECT PrimerNombreUsuario + ' '+ PrimerApellidoUsuario + ' ' + SegundoApellidoUsuario FROM " +
                    sqlCon.COMPAÑIA + ".NV_USUARIOS WHERE T1.UsuarioUltModificacionAdmin=CodigoUsuario) NombreUsuarioModificacion FROM "
            ...
            try
            {
                ...
                for (int i = 0; i < colsPKfechas; i++)
                {
                    if (colsPK > 0)
					{
                        ...
                        {
                            if (i + 1 < colsPKfechas)
                                where += " AND " + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i + 1] + "' AND ";
                            else
                                where += " AND " + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i + 1] + "'";
                        }
                        else
                            where += " AND " + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i + 1] + "'";
                    } 
                    else
					{
                        ...
                        {
                            if (i + 1 < colsPKfechas)
                                where = where + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i] + "' AND ";
                            else
                                where = where + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i] + "'";
                        }
                        else
                            where = where + listar.COLUMNAS_PK_FECHAS[i] + "=" + "'" + listar.VALORES_PK[i] + "'";
                    }
                ...
                if (lbOk)
				    lbOk = auditoriaDesdeEliminar(DateTime.Now, identificadorFormulario, listar, ref error);
        ...
        public static bool insertar(List<string> valoresParametros, Listado listar, string identificadorFormulario, ref string error, string auditoria = "si")
        {
            ...
            try
            {
                ...
                if (listar.COLUMNAS_IDENTITY_PK.Any())
                {
                    parametros = parametros.Where(x => !listar.COLUMNAS_IDENTITY_PK.Contains(x)).ToArray();
                    listar.COLUMNAS = string.Empty;
                    for (int i = 0; i < parametros.Length; i++)
                    {
                        if (i < parametros.Length-1)
                            listar.COLUMNAS += parametros[i].Trim() + ",";
                        else
                            listar.COLUMNAS += parametros[i].Trim();
                    }
                }

                string strSQL = "INSERT INTO " + listar.COMPAÑIA + "." + listar.TABLA + " (" + listar.COLUMNAS +
                    ",UsuarioCreacionAdmin,FechaCreacionAdmin,UsuarioUltModificacionAdmin,FechaUltModificacionAdmin) VALUES (";
                ...
                sqlCon.COMMAND.Parameters.AddWithValue("@FechaCreacion", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fff"));
                sqlCon.COMMAND.Parameters.AddWithValue("@UsuarioModificacion", sqlCon.USUARIO);
                sqlCon.COMMAND.Parameters.AddWithValue("@FechaModificacion", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fff"));
                ...
                if (auditoria == "si")
                    ...
                    //return true;// auditoriaDesdeInsert(parametros, DateTime.Now, identificadorFormulario, valoresParametros, listar, ref error);
                else
                    return true;
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (registro != "")
				{
                    ...
                    for (int i = 0; i < columnas.Length; i++)
                    {
                        ...
                        sqlCon.COMMAND.Parameters.AddWithValue("@FechaHora", fecha.ToString("yyyy-MM-ddTHH:mm:ss.fff"));
        ...
        public static bool ejecutarSentenciaParametros(string strSQL, List<string> parametros, ref string error)
        {
            ...
            for (int i = 0; i < parametros.Count; i++)
                if (parametros[i].Split(',')[1] == "null")
                    sqlCon.COMMAND.Parameters.AddWithValue(parametros[i].Split(',')[0], DBNull.Value);
                else
                    sqlCon.COMMAND.Parameters.AddWithValue(parametros[i].Split(',')[0], parametros[i].Split(',')[1]);
            ...
            {
                error = "[ejecutarSentenciaParametros] Ocurrió un error al ejecutar la sentencia:\n" + ex.Message;
                if (ex.Message.Contains("PK"))
                    error = "Ya existe un registro con el mismo código en la base de datos.";
        ...
        public static bool ejecutarSentenciaEscalarParametros(string strSQL, List<string> parametros, ref object dato, ref string error)
        {
            sqlCon.COMMAND = new SqlCommand();
            ...
            {
                sqlCon.COMMAND.CommandTimeout = 0;
                dato = sqlCon.ExecuteEscalar(strSQL, sqlCon.COMMAND);
                sqlCon.COMMAND.Dispose();
                sqlCon.COMMAND = null;
                return true;
            }
            ...
            {
                error = "[ejecutarSentenciaEscalarParametros] Ocurrió un error al ejecutar la sentencia:\n" + ex.Message;
                if (ex.Message.Contains("PK"))
                    error = "Ya existe un registro con el mismo código en la base de datos.";
        ...
        public static bool insertarFoto(string colegiado, byte[] archivo, ref string error)
        {
            sqlCon.COMMAND = new SqlCommand();
            try
            {
                string strSQL = "UPDATE " + Consultas.sqlCon.COMPAÑIA + ".NV_COLEGIADO set Foto = @Contenido WHERE IdColegiado = '" + colegiado + "'";
            ...
            {
                error = "[insertarFoto] Ocurrió un error guardando la imagen de la Aseguradora:\n" + ex.Message;
                return false;
            }
        }

        public static bool quitarFoto(string colegiado, ref string error)
        {
            sqlCon.COMMAND = new SqlCommand();
            try
            {
                ...
                sqlCon.COMMAND.Parameters.AddWithValue("@Contenido", SqlBinary.Null);
                sqlCon.EjecutarNonQuery(strSQL, sqlCon.COMMAND);
                sqlCon.COMMAND.Dispose();
                sqlCon.COMMAND = null;
                return true;
            ...
            {
                error = "[quitarFoto] Ocurrió un error guardando la imagen de la Aseguradora:\n" + ex.Message;
                return false;
            }
        }
        ...
        public static bool actualizar(List<string> valoresParametros,Listado listar,string identificadorFormulario,ref string error)
        {
            ...
            try
            {
                ...
                //return true;// auditoriaDesdeUpdate(parametros,DateTime.Now,identificadorFormulario,valoresParametros,listar,dt,ref error);
