...
namespace KOLEGIO
{
	public partial class frmFiltroRegentes : Form
	{
		...
		private void armarTablaRpt()
		{
			...
			listRPT.TABLA = "NV_RPT_REGENTES";
			nombreReporte = "RPT_Regentes.rpt";
		...
		private string armaFiltro()
		{
			...
			if (tipos.Count > 0)
			{
				...
				{
					hilera += "'" + tipos[i].ToString() + "',";
				}
				if (hilera != ")")
				{
					hilera = hilera.Remove(hilera.Length - 1, 1) + ")";
					sentencia += " CodigoTipo IN " + hilera;
				}
			}
