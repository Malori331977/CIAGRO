...
namespace KOLEGIO
{
    public partial class frmCobrosCanonesAnuales : Form
    {
        ...
        private void generarCanones()
        {
            ...
            foreach (DataGridViewRow row in dgvCanones.Rows)
            {
                if ((bool)row.Cells["Aplicar"].Value==true)
                {
                    bool OK = true;
        ...
        private void dgvCanones_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvCanones.Rows.Count > 0)
            {
                ...
                if (dgvCanones.CurrentCell.OwningColumn.Name == "Aplicar")
                {
                    if (e.RowIndex >= 0)
                    {
                        //if ((bool)dgvColegiados.Rows[e.RowIndex].Cells["colCheck"].Value == false)
                        if ((bool)dgvCanones.Rows[e.RowIndex].Cells["Aplicar"].Value == false)
                        {
                            //dgvColegiados.Rows[e.RowIndex].Cells["colCheck"].Value = true;
                            dgvCanones.Rows[e.RowIndex].Cells["Aplicar"].Value = true;
                        }
                        else
                            dgvCanones.Rows[e.RowIndex].Cells["Aplicar"].Value = false;
                ...
                if(Consultas.fillDataTable(sQuery,ref dtIngresos,ref error))
                {
                    ...
                    foreach (DataRow row in dtIngresos.Rows)
                    {
                        if (row["AñoUltimoCobro"].ToString().Equals(""))
                        {
                            ...
                            {
                                totalRegistros += 1;
                                dgvCanones.Rows.Add(false, row["IdColegiado"].ToString(), row["Cedula"].ToString(), row["NumeroColegiado"].ToString(),
                            ...
                            {
                                totalRegistros += 1;
                                dgvCanones.Rows.Add(false, row["Codigo"].ToString(), row["Cedula"].ToString(), string.Empty,
                                    row["Nombre"].ToString(), row["Email"].ToString(), row["AñoUltimoCobro"].ToString() != "" ? DateTime.Parse(row["AñoUltimoCobro"].ToString()).ToString("yyyy") : "", row["Cobrador"].ToString());
